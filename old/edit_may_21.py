...
def show_tiles(w, raw_tiles, raw_pointers):
    ...
    going = True
    while going:
        draw_tile(screen, monsters[cur_monster].tile(cur_tile))
...
def draw_tile(screen, tile):
    """
    This just draws a tile onto a given screen.
    """
    for i in range(0, tile.height()):
        for j in range(0, tile.width()):
            pygame.draw.rect(screen,COLORS[tile.tile(j,i)],(j*20,i*20,(j+1)*20,(i+1)*20),0)
...
def load_raw_tiles():
    """
    This, similar to the function above, will load all of the tiles from the file
