...
class animation:
    ...
    """
    def __init__(self, level, n, tiles, pointers):
        ...
        # First, open the dictionary that tells us where to look.
        #f = open("EGAPTR{0:0>2}.BIN".format(level),"rb")
        ...
        for i in range(0, NUM_TILES):
            ...
            #raw_offset = pointers[n * PTR_SIZE + i * 0x100:n * PTR_SIZE + i * 0x100 + 2]
            #raw_offset = f.read(2)
            #offset = ord(raw_offset[0]) + ord(raw_offset[1]) * 0x0100
            # This will tell you the first offset.
            offsets.append(offset) 

            # This moves to the next offset.
            f.read(0x100 - 2)
        ...
        for i in range(0,len(offsets)):
            self.tiles.append(tile(level,offsets[i]))
...
class tile:
    ...
    def __init__(self, level, offset):
        ...
        f = open("TANBIT{0:0>2}.BIN".format(level),"rb")


        # This feels very inefficient, if we want to open many tiles at a time. In this case, though, 
        # since the size of the files is so small anyhow, I doubt it matters in the least.

        # It is also worth noting that I'm cheating slightly here, which could have adverse effects.
        # In EGAPTRXX.BIN, offsets are given for _each_ of the tiles, not just for the first. For now,
        # I will leave this like this, but it should be noted that this could be a problem.
        f.read(offset)
        tile_1 = f.read(TILE_SIZE)
        tile_2 = f.read(TILE_SIZE)
        tile_3 = f.read(TILE_SIZE)
        tile_4 = f.read(TILE_SIZE)
        f.close()

        
        self.tile_data = []
        self.tile_data.extend(self.read_tiles(tile_1,tile_2))
        self.tile_data.extend(self.read_tiles(tile_3,tile_4))
...
def show_tiles(w, curlvl, raw_tiles, raw_pointers):
    """
    ...
    for i in range(0,MAX_ENEMIES):
        monsters.append(animation(curlvl,i,raw_tiles,raw_pointers))
    #cur_animation = animation(curlvl,0)
...
def load_raw_animation_data():
    ...
    for i in range(1, MAX_LEVELS):
        f = open("TANBIT{0:0>2}.BIN".format(i),"rb")
        tiles = f.read()
        if len(tiles) < len(output[-1]):
            tiles.append(output[-1][len(tiles):])
        output.append(tiles)
        f.close()

    pointers = []
    for i in range(0, MAX_LEVELS):
        f = open("EGAPTR{0:0>2}.BIN".format(i),"rb")
        pointers.append(f.read())
        f.close()

    return (tiles, pointers)
...
def main(w):
    ...
    while True:
        ...
        if ch == KEY_LEFT:
            ...
            show_tiles(w,curlvl, raw_tiles[curlvl-1], raw_pointers[curlvl - 1])
        else:
            if edit:
